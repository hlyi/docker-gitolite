...
if [ ! -e /etc/ssh/ssh_host_rsa_key ]; then
  ...
  HOSTKEY_DIR="/home/gitolite/repositories/.ssh/host-keys"
...
if [ -d ./.ssh ]; then
   chown -R gitolite:git ./.ssh

else
  CLIENT_DIR="/home/gitolite/repositories/.ssh/client"
  # .ssh does not exist; As an alternative, we allow the .ssh/client
  # folder from the repositories volume to be copied.
  if [ -d "$CLIENT_DIR" ]; then
    echo "Copying files from $CLIENT_DIR to /home/gitolite/.ssh"
    cp -pr $CLIENT_DIR ./.ssh
    chown -R gitolite:git ./.ssh
  fi
fi

# Always make sure the gitolite user has a private key you may
# use for mirroring setups etc.
if [ ! -f ./.ssh/id_rsa ]; then
   ssh-keygen -f /home/gitolite/.ssh/id_rsa  -t rsa -N ''
   echo "Here is the public key of the container's 'gitolite' user:"
   cat /home/gitolite/.ssh/id_rsa.pub
fi
...
if [ ! -f ./.ssh/known_hosts ]; then
    if [ -n "$TRUST_HOSTS" ]; then
        ...
        ssh-keyscan -H $TRUST_HOSTS > /home/gitolite/.ssh/known_hosts
    fi
fi
...
if [ ! -d ./.gitolite ] ; then
   ...
   if [ -d ./repositories ] ; then
       chown -R gitolite:git repositories
   fi
   ...
   if [ -n "$SSH_KEY" ]; then
       ...
       USER=gitolite bin/gitolite setup -a dummy
       ...
       rcfile=/home/gitolite/.gitolite.rc
       ...
       cd /home/gitolite/repositories/gitolite-admin.git && GL_LIBDIR=$(/home/gitolite/bin/gitolite query-rc GL_LIBDIR) PATH=$PATH:/home/gitolite/bin hooks/post-update refs/heads/master
   fi
else
